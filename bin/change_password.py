...
def change_user_password(password):
    ...
    hashed = Shell.stdout(
        sh.run(f"mkpasswd -m sha-512 {shlex.quote(password)}",
               sensitive=password))
...
def update_tpm2():
    ...
    result = sh.run(f"{nixos}/bin/tpm2.py enable",
...
def main():
    ...
    if change_user:
        if change_fde: change_user_password(new_password)
        else: change_user_password(ask_for_new_password())
