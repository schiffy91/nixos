...
class Installer:
    ...
    def install_nixos(cls):
        ...
        cmd = (f"nixos-install --flake {mount}{nixos}#{host}-{target} "
    ...
    def run_disko(cls, mode, args=""):
        ...
        mount = cls.get_mount_point()
        command = (f"nix --extra-experimental-features nix-command "
...
def main():
    ...
    if Interactive.confirm("Install NixOS?"): Installer.install_nixos()
    elif Interactive.confirm("Permission NixOS?"): Installer.permission_nixos()
