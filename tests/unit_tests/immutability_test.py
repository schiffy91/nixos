...
class TestRun:
    ...
    def test_stderr_logged_on_failure(self, capsys):
        r = mock_run(1)
        r.stderr = "permission denied\n"
        with patch("subprocess.run", return_value=r):
            immutability.run("bad-cmd")
            assert "permission denied" in capsys.readouterr().err
    def test_empty_lines_not_logged(self, capsys):
        r = mock_run()
        r.stdout = "\n  \n\nactual output\n  \n"
        with patch("subprocess.run", return_value=r):
            immutability.run("cmd")
            ...
            stdout_lines = [l for l in lines if "actual output" in l]
            assert len(stdout_lines) == 1
    def test_splits_string_to_list(self):
        with patch("subprocess.run", return_value=mock_run()) as m:
            immutability.run("btrfs subvolume delete /mnt/@root")
            args = m.call_args[0][0]
            assert args == ["btrfs", "subvolume", "delete", "/mnt/@root"]
    def test_returns_nonzero_on_failure(self):
        with patch("subprocess.run", return_value=mock_run(42)):
            assert immutability.run("failing-cmd") == 42
...
class TestAbort:
    ...
    def test_abort_logs_error_message(self, capsys):
        with patch("subprocess.run", return_value=mock_run()):
            with pytest.raises(SystemExit):
                immutability.abort("disk is full")
            err = capsys.readouterr().err
            assert "disk is full" in err
    def test_abort_exit_code_is_1(self):
        with patch("subprocess.run", return_value=mock_run()):
            with pytest.raises(SystemExit) as exc_info:
                immutability.abort("error")
            assert exc_info.value.code == 1
    def test_abort_cleans_mount_point(self):
        with patch("subprocess.run", return_value=mock_run()) as m:
            with pytest.raises(SystemExit):
                immutability.abort("error")
            cmds = [c[0][0] for c in m.call_args_list]
            assert any("rm" in str(c) and "-rf" in str(c) for c in cmds)
...
class TestBtrfs:
    ...
    def test_snapshot_deletes_existing_dst(self):
        calls = []
        ...
        delete_calls = [c for c in calls
                        if isinstance(c, list) and "delete" in c]
        assert len(delete_calls) >= 1
    def test_snapshot_cmd_format(self):
        with patch("subprocess.run", return_value=mock_run()) as m:
            with patch("os.path.isdir",
                       side_effect=lambda p: "src" in p):
                immutability.btrfs_snapshot("/mnt/src", "/mnt/dst")
        snapshot_calls = [
            c[0][0] for c in m.call_args_list
            if isinstance(c[0][0], list) and "snapshot" in c[0][0]
        ]
        assert len(snapshot_calls) == 1
        assert snapshot_calls[0] == [
            "btrfs", "subvolume", "snapshot", "/mnt/src", "/mnt/dst"]
...
class TestMount:
    ...
    def test_mount_creates_directory(self):
        with patch("os.path.exists", return_value=True):
            with patch("subprocess.run", return_value=mock_run()) as m:
                immutability.mount_subvolumes("/dev/sda")
                cmds = [c[0][0] for c in m.call_args_list]
                assert any("mkdir" in str(c) for c in cmds)
    def test_mount_uses_btrfs_type(self):
        with patch("os.path.exists", return_value=True):
            with patch("subprocess.run", return_value=mock_run()) as m:
                immutability.mount_subvolumes("/dev/sda")
                cmds = [c[0][0] for c in m.call_args_list]
                mount_cmds = [c for c in cmds
                              if isinstance(c, list) and "mount" in c]
                assert any("btrfs" in str(c) for c in mount_cmds)
...
class TestRecovery:
    ...
    def test_sentinel_file_path(self):
        m = mock_open()
        with patch("builtins.open", m):
            immutability.create_sentinel("/mnt/@snapshots/@root/CURRENT")
            m.assert_called_once_with(
                "/mnt/@snapshots/@root/CURRENT/.boot-ready",
                "w", encoding="utf-8")
    def test_recovery_checks_boot_ready_file(self):
        with patch("os.path.isdir", return_value=True):
            with patch("os.path.isfile") as isfile:
                isfile.return_value = True
                immutability.check_recovery_needed("/snap")
                isfile.assert_called_once_with("/snap/.boot-ready")
...
class TestCopyPersistentFiles:
    ...
    def test_copy_uses_list_args(self):
        with patch("subprocess.run", return_value=mock_run()) as m:
            ...
            assert "shell" not in kwargs
    def test_copy_trailing_slash_on_source(self):
        with patch("subprocess.run", return_value=mock_run()) as m:
            immutability.copy_persistent_files(
                "/prev", "/curr", "/filter.txt",
            )
            args = m.call_args[0][0]
            assert "/prev/" in args
    def test_copy_rsync_flags(self):
        with patch("subprocess.run", return_value=mock_run()) as m:
            immutability.copy_persistent_files(
                "/prev", "/curr", "/filter.txt",
            )
    ...
    def test_copy_filter_format(self):
        with patch("subprocess.run", return_value=mock_run()) as m:
            immutability.copy_persistent_files(
                "/prev", "/curr", "/nix/store/abc-filter",
            )
            args = m.call_args[0][0]
            assert "--filter=. /nix/store/abc-filter" in " ".join(args)
...
class TestMain:
    ...
    def test_parses_filter_from_pair_args(self, monkeypatch):
        ...
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume") as proc:
                with patch("lib.immutability.unmount_subvolumes"):
                    immutability.main()
    ...
    def test_thread_error_propagation(self, monkeypatch):
        ...
        def failing_process(pair, *a):
            if "@root" in pair:
                raise SystemExit(1)
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume",
                       side_effect=failing_process):
                with patch("lib.immutability.unmount_subvolumes"):
                    with pytest.raises(SystemExit):
                        immutability.main()
    def test_thread_catches_runtime_error(self, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "reset", "@root=/:/f1", "@home=/home:/f2",
        ])
        def failing_process(pair, *a):
            if "@root" in pair:
                raise RuntimeError("btrfs I/O error")
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume",
                       side_effect=failing_process):
                with patch("lib.immutability.unmount_subvolumes"):
                    with pytest.raises(SystemExit):
                        immutability.main()
    def test_thread_catches_os_error(self, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "reset", "@root=/:/f1", "@home=/home:/f2",
        ])
        ...
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume",
                ...
                with patch("lib.immutability.unmount_subvolumes"):
                    with pytest.raises(SystemExit):
                        immutability.main()
    def test_thread_catches_permission_error(self, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "reset", "@root=/:/f1", "@home=/home:/f2",
        ])
        ...
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume",
                ...
                with patch("lib.immutability.unmount_subvolumes"):
                    with pytest.raises(SystemExit):
                        immutability.main()
    def test_thread_both_fail_reports_both(self, monkeypatch, capsys):
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "reset", "@root=/:/f1", "@home=/home:/f2",
        ])
        def failing_process(pair, *a):
            raise RuntimeError(f"fail on {pair}")
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume",
                       side_effect=failing_process):
                with patch("lib.immutability.unmount_subvolumes"):
                    with pytest.raises(SystemExit):
                        immutability.main()
                    err = capsys.readouterr().err
                    assert "@root" in err or "@home" in err
    def test_thread_success_no_abort(self, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "reset", "@root=/:/f1", "@home=/home:/f2",
        ])
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume"):
                with patch("lib.immutability.unmount_subvolumes"):
                    immutability.main()  # should not raise
    def test_disabled_mode_still_unmounts(self, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "disabled", "@root=/",
        ])
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.unmount_subvolumes") as um:
                immutability.main()
                um.assert_called_once()
    def test_disabled_mode_logs_message(self, monkeypatch, capsys):
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "disabled", "@root=/",
        ])
    ...
    def test_filter_parsing_colon_in_filter(self, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "reset", "@root=/:/nix/store/abc-filter",
        ])
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume") as proc:
                with patch("lib.immutability.unmount_subvolumes"):
                    immutability.main()
                    filter_files = proc.call_args[0][4]
                    assert filter_files["@root"] == "/nix/store/abc-filter"
    def test_mount_called_before_process(self, monkeypatch):
        order = []
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "reset", "@root=/:/f1",
        ...
        with patch("lib.immutability.mount_subvolumes",
                   side_effect=track_mount):
            with patch("lib.immutability.process_subvolume",
                       side_effect=track_process):
                with patch("lib.immutability.unmount_subvolumes"):
                    immutability.main()
                    assert order == ["mount", "process"]
    def test_unmount_called_after_process(self, monkeypatch):
        order = []
        monkeypatch.setattr("sys.argv", [
            "immutability", "/dev/sda", "@snapshots", "CLEAN",
            "reset", "@root=/:/f1",
        ...
        with patch("lib.immutability.mount_subvolumes"):
            with patch("lib.immutability.process_subvolume",
                       side_effect=track_process):
                with patch("lib.immutability.unmount_subvolumes",
                           side_effect=track_unmount):
                    immutability.main()
                    assert order == ["process", "unmount"]
