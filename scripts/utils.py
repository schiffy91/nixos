...
class Interactive:
    ...
    def ask_for_host_path(cls):
        ...
        formatted_hosts_paths = [ f"{cls.sh.basename(host_path).replace('.nix', '')} ({cls.sh.parent_name(cls.sh.parent_name(host_path))} {cls.sh.parent_name(host_path)})"  for host_path in hosts_paths ]
