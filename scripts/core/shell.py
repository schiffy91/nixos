...
class Shell:
    ...
    def run(self, cmd, env="", sudo=True, capture_output=True,
            check=True, sensitive=None):
        if self.chroots:
        ...
        try:
            ...
            if e.stdout:
                print(f"\033[90mLOG: {e.stdout}\033[0m")
            if e.stderr:
                print(
                    f"\033[38;5;208mERROR: {e.stderr}\033[0m",
                    file=sys.stderr,
                )
            raise
        return result
    ...
    def find(self, path, pattern="*", ignore_pattern=None,
             ignore_files=False, ignore_directories=False):
    ...
    def find_directories(self, path, pattern="*", ignore_pattern=None):
        return self.find(
            ...
            ignore_pattern=ignore_pattern, ignore_files=True
        )

    def find_files(self, path, pattern="*", ignore_pattern=None):
        return self.find(
            path, pattern=pattern,
            ignore_pattern=ignore_pattern, ignore_directories=True
        )
    ...
    def file_write(self, path, string, sensitive=None):
        if sensitive:
            string = string.replace(sensitive, "***")
        self.rm(path)
        self.mkdir(self.dirname(path))
        if self.chroots:
            path = f"{self.chroots[-1]}{path}"
    ...
    def json_read(self, path):
        try:
            return json.loads(self.file_read(path))
...
def chrootable(cls):
    if not hasattr(cls, "sh"):
        raise TypeError(
            f"Class {cls.__name__} must have a 'sh' attribute "
            "to be chrootable."
        )
