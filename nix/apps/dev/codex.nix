  ...
  mkHomeFiles = user:
    lib.mkIf user.homeManager.enable {
      home-manager.users.${user.username} = { config, ... }: {
        home.file.".codex/skills/nixos-init".source =
          config.lib.file.mkOutOfStoreSymlink "${config.home.homeDirectory}/.agents/skills/nixos-init";
      };
    };
in
{
  config = lib.mkIf (config.settings.apps.enable && config.settings.apps.dev.enable && config.settings.apps.codex.enable) (lib.mkMerge [
