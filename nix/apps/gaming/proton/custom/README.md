...
| `ntdll-system-thread-signals` | `0001` | Active. Records the libc thread pointer for `PsCreateSystemThread` threads (GE `em-fixups/0030`) so `init_handler` can reload `%fs` in WoW64 processes; without it every 32-bit process with a winewayland dispatcher (Battle.net.exe and its CEF renderers) segfaulted in the SIGQUIT handler at exit, leaving core dumps and DrKonqi notifications. |
...
| `win32u-managed-swapchain-clip` | `0001` | Active. Marks a window clipped-from-parent while a managed dmabuf swapchain presents into it, so ancestor SHM surfaces punch the client area out; without it a composition swap chain bound to a child HWND (Battle.net's login window) is hidden behind the opaque parent buffer. |
