...
def btrfs_delete_recursively(path):
    ...
    for line in result.stdout.splitlines():
        parts = line.split()
        if len(parts) >= 9:
            subvol = os.path.join(os.path.dirname(path), " ".join(parts[8:]))
...
def copy_persistent_files(previous, current, filter_file):
    ...
    if run(["rsync", "-aHAX", "--numeric-ids", "--delete",
            f"--filter=. {filter_file}", f"{previous}/", current]) != 0:
...
def reset_subvolume(name, _mount_point_arg, snapshots_name, clean_name,
    ...
    if not os.path.isdir(penultimate): btrfs_snapshot(clean, penultimate)
    if not os.path.isdir(previous): btrfs_snapshot(clean, previous)
